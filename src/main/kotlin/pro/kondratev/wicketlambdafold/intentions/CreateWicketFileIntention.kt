...
abstract class CreateWicketFileIntention : BaseElementAtCaretIntentionAction() {
    ...
    companion object {
        ...
        private val logger = Logger.getInstance(CreateWicketFileIntention::class.java.simpleName)
    }

    /**
     * Used as a criteria for availability of this action
     */
    fun hasNoResourceFile(ofExtension: String, element: PsiElement): Boolean {
    ...
    protected fun isSubclassOfApplicableClasses(c: PsiClass?, fuse: Int): Boolean {
        ...
        return c.supers.any {
            APPLICABLE_CLASSES.contains(it.qualifiedName) || isSubclassOfApplicableClasses(it, fuse - 1)
        }
    }
    ...
    fun createPropertiesFile(project: Project, element: PsiElement) {
    ...
    fun createHtmlFile(project: Project, element: PsiElement) {
        assert(element.parent is PsiClass)
        val psiClass = element.parent as PsiClass
